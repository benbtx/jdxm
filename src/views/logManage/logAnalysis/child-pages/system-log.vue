<template>
  <div class="system-log" v-kr-loading="loading">
    ...
    <div style="padding: 14px">
      ...
      >
        <table-info :inOneCell="false" :data="dialogData.row" :cells="dialogData.cells"></table-info>
      </el-dialog>
    </div>
  </div>
</template>
...
<script>
import { systemExpCfg, systemDetailCells, systemLogTableCfg } from './components/config'
import {
  fetchLogAnalysisLogList, deleteAnalysisLogByIds,
...
export default {
  ...
  data () {
    return {
      ...
      dialogData: {
        ...
        visible: false,
        row: {},
        cells: systemDetailCells
      },
      rows: [],
      total: 0,
      activeRow: {},
      selectedRows: [],
      columns: systemLogTableCfg
    }
  },
  computed: {
    buttons () {
  ...
  methods: {
    ...
    async fnGetData (isExportAll) {
      ...
      const result = await fetchLogAnalysisLogList(params, 'system')
      const {
        data: { rows = [] }
      } = result
      return rows
    ...
    async doDeleteAll () {
      ...
      const result = await deleteAnalysisLogByCondition(params, 'system')
      this.loading = false
      if (result.code === 0) {
        this.$message.success('删除成功！')
        this.fetchTableData()
