<template>
  <div class="message-log" v-kr-loading="loading">
...
import searchBar from './components/message-search-bar.vue'
...
export default {
  ...
  data () {
    return {
      ...
      dialogData: {
        ...
        visible: false,
        row: {},
        cells: messageDetailCells
      },
      rows: [],
      total: 0,
      activeRow: {},
      selectedRows: [],
      columns: messageTableCfg
    }
  },
  computed: {
    buttons () {
  ...
  methods: {
    ...
    async fnGetData (isExportAll) {
      ...
      const result = await fetchLogAnalysisLogList(params, 'command')
      const {
        data: { rows = [] }
      } = result
      return rows
    ...
    async doDeleteAll () {
      ...
      const result = await deleteAnalysisLogByCondition(params, 'command')
      this.loading = false
      if (result.code === 0) {
        this.$message.success('删除成功！')
        this.fetchTableData()
