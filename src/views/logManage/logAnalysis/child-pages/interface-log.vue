<template>
  ...
  <div class="interface-log" v-kr-loading="loading">
...
import {
  interfaceExpCfg,
  interfaceDetailCells,
  interfaceTableCfg
} from './components/config'
import searchBar from './components/interface-search-bar.vue'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      dialogData: {
        ...
        visible: false,
        row: {},
        cells: interfaceDetailCells
      },
      rows: [],
      total: 0,
      activeRow: {},
      selectedRows: [],
      columns: interfaceTableCfg
    }
  },
  computed: {
    buttons () {
  ...
  methods: {
    ...
    async fnGetData (isExportAll) {
      ...
      const result = await fetchLogAnalysisLogList(params, 'interface')
      const {
        data: { rows = [] }
      } = result
      return rows
    ...
    async doDeleteAll () {
      ...
      const result = await deleteAnalysisLogByCondition(params, 'interface')
      this.loading = false
      if (result.code === 0) {
        this.$message.success('删除成功！')
        this.fetchTableData()
