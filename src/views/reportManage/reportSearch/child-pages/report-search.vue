...
<script>
import { getReportList, reportViewList, getRole, getUser, reportAuth, reportDelete, reportEnable, reportCheck, setPreviewLog } from '@/projectModule/api/reportManage/reportManage.js'
  ...
  data () {
    ...
    return {
      ...
      columns: [
        ...
        // { prop: 'reportStatus', label: '状态' ,
        //   render (h, row) {
        //     // $shelf.ruleForm.status=row.row.reportStatus=="启用中";
        //     let status=row.row.reportStatus==true;
        //     return (
        //       <el-switch
  ...
  computed: {
    buttons () {
      ...
      return [
        ...
        {
          permission: 'reportmanage:search:preview',
          type: 'primary',
          plain: false,
          text: '预览',
          icon: 'el-icon-view',
          ...
          fn: () => {
            ...
            const data = {
              reportId: activeRow.id// this.selection[0].id
            }
            reportCheck(data).then(data => {
              if (data.success) {
                if (data.data == 'disEnable') {
                  ...
                  setPreviewLog(data).then(data => {
                    if (data.success) {
                      ...
                      _this.$message('日志记录失败')
                    }
                  })

                  window.open(activeRow.token)
                  // // window.open(activeRow.token,"_blank","toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=yes");
                  // _this.$router.push({
  ...
  methods: {
    ...
    getList () {
      const data = {
        ...
        roleId: this.ruleForm.role || 1,
        userId: this.ruleForm.user || 1
        // "userName":  this.ruleForm.user||"张三"
      }
      reportViewList(data).then(data => {
        if (data.success) {
          this.rows = data.data
